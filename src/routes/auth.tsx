...
function AuthPage() {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // Save the logged-in user so other pages can access it.
      localStorage.setItem("smartcheck_user", JSON.stringify(data.user));
      ...
      console.error(err);
      setError(
        "Unable to connect to the SmartCheck server. Make sure the backend is running."
      );
    } finally {
      setLoading(false);
    }
      ...
      >
        ...
        <div className="relative z-10 text-primary-foreground max-w-md">
          <h2 className="font-display text-4xl font-bold leading-tight">
            Rubric-aligned feedback, generated in seconds.
          </h2>

          <p className="mt-4 text-primary-foreground/80">
            Upload your rubric and student outputs. SmartCheck retrieves your
            criteria and produces consistent, teacher-reviewable feedback.
          </p>
        ...
        <div className="text-xs text-primary-foreground/70 relative z-10">
          © 2026 SmartCheck · New Era University
        </div>
        ...
        <form onSubmit={onSubmit} className="w-full max-w-sm">
          ...
          <div className="mt-8 space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full h-11 rounded-lg text-primary-foreground font-medium transition-transform active:scale-[0.99] disabled:opacity-60 disabled:cursor-not-allowed"
              style={{
